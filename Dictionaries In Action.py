    ...
    while dictionary['gone'] == "y":
        ...
        if x.lower() == "a":
            ...
            print("Today, you withdrew $%d from your account and spent a "
                  "total of $%d."%
                  (dictionary['walletTotal'], dictionary['costTotal']))
            
            if dictionary['endSum'] < 0:
                print("Somehow, you managed to go into debt... $%d, to be "
                      "exact. Never go shopping alone again."%
                      (dictionary['endSum']))
...
def afford():
    ...
    if dictionary['wallet'] - int(dictionary['cost']) > 0:
        print("You make the purchase.\n")
        dictionary['wallet'] = dictionary['wallet'] - int(dictionary['cost'])
        print("Your wallet now contains $%d."% (dictionary['wallet']))
        
    elif dictionary['wallet'] - int(dictionary['cost']) < 0:
        print("You cannot afford to make this purchase.\n")
        print("Your wallet still contains $%d."% (dictionary['wallet']))
